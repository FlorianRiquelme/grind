...
`job::from_issue_json` (`src/job.rs:120`) reads `| key | value |` rows anywhere in the body:
...
**The first seven are required and refused at dispatch if missing.** A row reading `none`, `-`,
